...
int n, cnt, inorder[N], postorder[N];
int res[N][N];
queue <int> q;
struct node {
    int value, l = -1, r = -1;
...
int build(int inorder_l, int inorder_r, int postorder_l, int postorder_r) {
    ...
    int root = postorder[postorder_r], a = cnt ++ ;
    nodes[a].value = root;
    int pos = inorder_l;
    // 获取中序遍历，根节点所在位置
    while (inorder[pos] != root) pos++;
    // 构建左子树
    nodes[a].l = build(inorder_l, pos - 1, postorder_l, postorder_l + (pos - 1 - inorder_l));
    // 构建右子树
    nodes[a].r = build(pos + 1, inorder_r, postorder_l + pos - inorder_l, postorder_r - 1);
    return a;
}
